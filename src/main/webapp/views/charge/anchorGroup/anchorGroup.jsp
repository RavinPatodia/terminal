...
park.modalInit();
UIIdleTimeout.init();
$('.multi-select').multiSelect({keepOrder: true,selectableOptgroup: true,selectableHeader: "<div class='custom-header'>可选的长期规则</div>",
  	  selectionHeader: "<div class='custom-header'>选中的长期规则</div>"});
...
var dataTable_config = {//jquery_datatable插件配置
    //下面设置数据栏目是否可排序
    "bServerSide": true,
    "sAjaxSource": pageUrl.charge_anchorGroup_list,
...
function retrieveData( sSource, aoData, fnCallback ) {     
    //查询条件称加入参数数组     
    var anchorGroup=$("#anchorGroup").val();
    var name =$("#name").val();
    $.ajax( {     
        type: "POST",
        url: sSource,   
        dataType:"json",  
        data: "jsonParam="+JSON.stringify(aoData)+"&historyFlag=0&anchorGroupId="
...
$(function() {
	...
	var editAndDblUrl = pageUrl.charge_anchorGroup_editAndDbl;
	...
	$('.tb_del').delUseDialog(dataSource,pageUrl.charge_anchorGroup_del,"长期规则组",preDel);
});
...
var preDel = function(ids){
	...
	var warnMsg="";
	$.ajax({
	        url: pageUrl.charge_anchorGroup_delValidate+ids,
	        async: false,
	        ...
	        success: function(data) {
	        	...
	        	warnMsg = notPermission+"被收费规则"+associate+"使用中，不允许被删除";
	        }
		  }); 
	if(notPermission!=""){
	...
	obj.live('dblclick',function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				var recs = formObj.anchorRentModelsInGroup;
				if(obj.success){
					$(showForm,dialog).fill(formObj);//表单填充插件
					dialog.modal('show');
					anchorGroupRecs(recs);
				}
				else{
				}
			}
