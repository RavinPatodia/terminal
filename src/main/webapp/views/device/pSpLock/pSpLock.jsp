...
<body class="page-header-fixed page-quick-sidebar-over-content ">
	...
	<div class="page-container">
		...
		<div class="page-content-wrapper">
					...
					<div class="portlet-body" style="position:relative;">
						<div class="btn-group" role="group" aria-label="...">
							...
							<button type="button" class="btn btn-sm btn-default tb-enable">
								<span class="glyphicon glyphicon-ok-circle"></span> 开启
							</button>
						</div>
			...
			<!--这个页面的弹窗都在这里 start-->
			<jsp:include page="/views/device/pSpLock/add.jsp" />
			<jsp:include page="/views/device/pSpLock/edit.jsp" />
			<!--这个页面的弹窗都在这里 end-->
		</div>
	</div>
	<!-- END CONTAINER -->
...
//FormiCheck.init();
	...
	(function () {
		var dataTable_config = {//jquery_datatable插件的默认参数配置
		    //下面设置数据栏目是否可排序
		    "bServerSide": true,
		    "sAjaxSource": pageUrl.device_pSpLock_list,
		    ...
		    "columnDefs": [
				...
				{  aTargets: [0],
					   ...
					   return '<input type="checkbox" class="checkboxes" data-name="'+row.lockId+'" data-state="'+row.lockState+'" data-linestate="'+row.lineState+'" value="'+data+'"/>';
					}
				}
				,
			    {   aTargets: [7],
		...
		function retrieveData( sSource, aoData, fnCallback ) {     
		    ...
		    $.ajax( {     
		        ...
		        url: sSource,   
		        dataType:"json",  
		        data: "jsonParam="+JSON.stringify(aoData)+"&lockId="
		        +lockId+"&brand="+brand+"&model="+model+"&lockState="+lockState,  
...
var initDrop=function(){
	$('.pspId').select2({
        ...
        formatSearching: "查询中...",
        ajax: {
            url: pageUrl.device_pSpLock_getPspId+"t="+ Math.random(),
            dataType: 'json',
            delay: 500,
...
$(function() {
		...
		var editAndDblurl = pageUrl.device_pSpLock_editAndDbl;
		add.init();//add modal初始化
		edit.init(dataSource,$("#modify"),editAndDblurl);
		$('.tb_del').delUseDialog(dataSource,pageUrl.device_pSpLock_del,"车位锁管理");
		$('.tb-enable').openUseDialog(dataSource,pageUrl.device_pSpLock_enable,"车位锁管理");
		$('.tb_disabled').closeUseDialog(dataSource,pageUrl.device_pSpLock_disable,"车位锁管理");
	});
//验证表单的方法
function check(form){
