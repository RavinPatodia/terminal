...
<div id="modify3" class="modal fade" tabindex="-1" data-width="400" data-backdrop="static" data-keyboard="false"> <!--修改 start-->
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
							<form class="form-horizontal" role="form" id="editTerminal">
								<div class="form-body">
									...
									<div class="form-group">
										<label class="col-md-4 control-label">终端机名称</label>
										<div class="col-md-6">
									...
									<div class="form-group">
									 ...
									 <div class="form-group">
									<label class="col-md-4 control-label">车道配置</label>
									<div class="col-md-6">
										<select multiple="multiple"  class="multi-select"  name="drivewaySelect">
										</select>
									</div>
								</div>
				...
				<div class="modal-footer">
					...
					<button type="button"  id="editTerminal_save" class="btn green">保存</button>
				</div>
			</div>
		</div>
	</div><!--修改 end-->
...
var editTerminal=function(){
	...
	var bindClick=function(dataSource,modal,url){
		...
		if(!dataSource)//如果绑定的表格不对的话，直接返回（如果插件无效，且没有错误提示的话那就是这个有问题了）
			return;
		$(".terminal_modify").bind('click',function(){//按钮绑定点击事件
			...
			var id = $(".checkboxes:checked").val(); //获取编辑的那种中选中的checkbox，获取checkbox的值（这里传入的是 id)
				$.ajax({
					...
					success : function(data) {
						editTerminal.callback(data, modal);
					},
	...
	var bindSubmit = function(){
		$("#editTerminal_save").on("click",function(){
			if(!check($("#editTerminal")).form()) return;
			if($('.has-error').length<=0){
			 $.ajax({
		       type: "POST",
		       url: pageUrl.device_deviceAll_edit,
		       data:$('#editTerminal').serialize(),// 你的formid
		       async: false,
	...
	var bindDataAndOpenModal=function(data,dialog){
		...
		if(obj.success){
			$("#editTerminal",dialog).fill(obj.object);//表单填充插件
			/* if(direction==0)
				$('#edit_direction0').iCheck('check');
			else if(direction==1)
				$('#edit_direction1').iCheck('check');
			$("#editTerminal").find(".bayonetName > a > span").eq(0).text(formObj.bayonetName); */
			$('#editTerminal .multi-select').multiSelect("addParkOptionAndSelectOption",formObj.selects);
			dialog.modal('show');
		}
		else{
			alert(obj.msg);
