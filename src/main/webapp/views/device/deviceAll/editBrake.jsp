		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
							<form class="form-horizontal" role="form" id="editBrake">
								<div class="form-body">
									<div class="form-group">
										<label class="col-md-4 control-label">闸机编号</label>
										<div class="col-md-6">
											<input type="text" class="form-control" readonly="readonly" name="deviceId" fill="obj.deviceId">
										</div>  
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<input type="text" class="form-control" name="open" fill="obj.open">
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-4 control-label">关信号</label>
										<div class="col-md-6">
											<input type="text" class="form-control" name="close" fill="obj.close">
										</div>
									</div>
				...
				<div class="modal-footer">
					...
					<button type="button"  id="editBrake_save" class="btn green">保存</button>
				</div>
			</div>
		</div>
	</div><!--修改 end-->
...
var editBrake=function(){
	...
	var bindSubmit = function(){
		$("#editBrake_save").on("click",function(){
			if(!check($("#editBrake")).form()) return;
			if($('.has-error').length<=0){
			 $.ajax({
		       type: "POST",
		       url: pageUrl.device_deviceAll_edit,
		       data:$('#editBrake').serialize(),// 你的formid
		       async: false,
	...
	var bindDataAndOpenModal=function(data,dialog){
		...
		if(obj.success){
			$("#editBrake",dialog).fill(obj.object);//表单填充插件
			/* if(direction==0)
				$('#edit_direction0').iCheck('check');
			else if(direction==1)
				$('#edit_direction1').iCheck('check');
			$("#editBrake").find(".bayonetName > a > span").eq(0).text(formObj.bayonetName); */
			dialog.modal('show');
		}
		else{
			alert(obj.msg);
