...
<div id="modify5" class="modal fade" tabindex="-1" data-width="400" data-backdrop="static" data-keyboard="false"> <!--修改 start-->
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
							<form class="form-horizontal" role="form" id="editDataMaster">
								<div class="form-body">
				...
				<div class="modal-footer">
					...
					<button type="button"  id="editDataMaster_save" class="btn green">保存</button>
				</div>
			</div>
		</div>
	</div><!--修改 end-->
...
var editDataMaster=function(){
			...
			var id = $(".checkboxes:checked").val(); //获取编辑的那种中选中的checkbox，获取checkbox的值（这里传入的是 id)
				$.ajax({
					...
					success : function(data) {
						editDataMaster.callback(data, modal);
					},
	...
	var bindSubmit = function(){
		$("#editDataMaster_save").on("click",function(){
			if(!check($("#editDataMaster")).form()) return;
			if($('.has-error').length<=0){
			 $.ajax({
		       type: "POST",
		       url: pageUrl.device_deviceAll_edit,
		       data:$('#editDataMaster').serialize(),// 你的formid
		       async: false,
	...
	var bindDataAndOpenModal=function(data,dialog){
		...
		if(obj.success){
			$("#editDataMaster",dialog).fill(obj.object);//表单填充插件
			/* if(direction==0)
				$('#edit_direction0').iCheck('check');
			else if(direction==1)
				$('#edit_direction1').iCheck('check');
			$("#editDataMaster").find(".bayonetName > a > span").eq(0).text(formObj.bayonetName); */
			dialog.modal('show');
		}
		else{
			alert(obj.msg);
