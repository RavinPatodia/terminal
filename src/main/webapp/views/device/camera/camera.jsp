...
<body class="page-header-fixed page-quick-sidebar-over-content ">
	...
	<div class="page-container">
		...
		<div class="page-content-wrapper">
			...
			<!--这个页面的弹窗都在这里 start-->
			<jsp:include page="/views/device/camera/add.jsp" />
			<jsp:include page="/views/device/camera/edit.jsp" />
			<jsp:include page="/views/device/camera/show.jsp" />
			<!--这个页面的弹窗都在这里 end-->
		</div>
	</div>
	<!-- END CONTAINER -->
...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/pageLevelJs/camera/cameraGlobal.js"></script><!--一些自己写的插件的-->
...
initDrop();
park.modalInit();
deviceSelect2.initAllDrivewaySelect();
UIIdleTimeout.init();
//那个表格的初始化
;(function () {
var dataTable_config = {//jquery_datatable插件的默认参数配置
    //下面设置数据栏目是否可排序
    "bServerSide": true,
    "sAjaxSource": pageUrl.device_camera_list,
    ...
    "aoColumns" : [ 
    {
        ...
        "mDataProp" : "lightState",
        "sWidth": "150px"
    },{
        "mDataProp" : "cameraState",
        "sWidth": "150px"
    },{
        "mDataProp" : "lineState",
        "sWidth": "150px"
	...
	},
	{
		...
		{  aTargets: [0],
		   ...
		   return '<input type="checkbox" class="checkboxes" data-name="'+row.cameraId+'" data-state="'+row.cameraState+'" data-linestate="'+row.lineState+'" value="'+data+'"/>';
		   }
...
function retrieveData( sSource, aoData, fnCallback ) {     
    ...
    var type =$("#type").val();
    var bayonetId =$("#bayonetId").val();
  /*   var brand =$("#brand").val();
    var model =$("#model").val(); */
    var cameraState =$("#cameraState").val();
    var lightState =$("#lightState").val();
    var drivewayId =$("#driveWayPK").val();    
    $.ajax( {     
        type: "POST",
        url: sSource,   
        dataType:"json",  
        data: "jsonParam="+JSON.stringify(aoData)+"&cameraId="
        +cameraId+"&type="+type+"&bayonetId="+bayonetId+"&cameraState="+cameraState+"&lightState="+lightState+"&drivewayId="+drivewayId,
...
var initDrop=function(){
	...
	$('.car_type').select2({
        ...
        formatSearching: "查询中...",
        ajax: {
            url: pageUrl.caradmission_car_typeName,
            dataType: 'json',
            delay: 500,
	...
	$('.car_color').select2({
        ...
        formatSearching: "查询中...",
        ajax: {
            url: pageUrl.caradmission_car_color,
            dataType: 'json',
            delay: 500,
            data: function (term, page) {
                return {};
	...
	$('.car_module').select2({
        ...
        formatSearching: "查询中...",
        ajax: {
            url: pageUrl.caradmission_car_module,
            dataType: 'json',
            delay: 500,
            data: function (term, page) {
                return {};
...
$(function() {
		...
		var editAndDblurl = pageUrl.device_camera_editAndDbl;
		add.init();//add modal初始化
		edit.init(dataSource,$("#modify"),editAndDblurl);
		$('.tb_del').delUseDialog(dataSource,pageUrl.device_camera_del,"摄像头管理");
		$('.tb_enable').openUseDialog(dataSource,pageUrl.device_camera_enable,"摄像头管理");
		$('.tb_disabled').closeUseDialog(dataSource,pageUrl.device_camera_disable,"摄像头管理");
